...
export const ProductCrud = () => {
    ...
    const createProduct = async () => {
        ...
        .post('http://192.168.137.49:8080/product', 
        {
            categoryId: 1,
            name, 
            price: 13,
    ...
    // ... (código anterior)
        ...
        const listProduct = async () => {
            ...
            console.log(token);
            try {
                const resultProduct = await axios.get('http://192.168.137.49:8080/product', {
                    headers: {
                        Authorization: token,
                        'Content-Type': 'application/json',
                    },
    ...
    const handleOnChangePrice = (event: NativeSyntheticEvent<TextInputChangeEventData>) => {
        setPrice(event.nativeEvent.text)
    }
    
    const handleOnChangeImage = (event: NativeSyntheticEvent<TextInputChangeEventData>) => {
